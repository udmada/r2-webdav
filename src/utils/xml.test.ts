...
describe("parseXmlProperties", () => {
	it("should parse simple properties", () => {
		...
		expect(result.byName.get("displayname" as any)).toBe("Test");
	});
	...
	it("should parse namespaced properties", () => {
		...
		expect(result.byName.get("d:displayname" as any)).toBe("Test");
	});
	...
	it("should parse multiple properties", () => {
		...
		expect(result.byName.get("displayname" as any)).toBe("Test");
		expect(result.byName.get("creationdate" as any)).toBe("2024-01-01");
	});
	...
	it("should handle properties with complex content", () => {
		...
		const result = parseXmlProperties(xml);
		expect(result.byName.get("displayname" as any)).toBe("Test Data");
	});
	...
	it("should handle empty properties", () => {
		...
		const result = parseXmlProperties(xml);
		expect(result.byName.get("displayname" as any)).toBe("");
	});
...
describe("extractSetRemoveProperties", () => {
	it("should extract set properties", () => {
		...
		expect(result.set.get("displayname" as any)).toBe("New Name");
	...
	it("should extract remove properties", () => {
		...
		expect(result.remove).toContain("displayname" as any);
	});
	...
	it("should extract both set and remove properties", () => {
		...
		const result = extractSetRemoveProperties(xml);
		expect(result.set.get("displayname" as any)).toBe("New Name");
		expect(result.remove).toContain("customfield" as any);
	});
