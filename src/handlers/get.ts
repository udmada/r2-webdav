...
import { sanitizeConditionalHeaders } from "../utils/headers";
...
const handleFileDownload = (
  ...
  Effect.gen(function* () {
    // Sanitize conditional headers to fix common ETag formatting issues
    const sanitizedHeadersObj = sanitizeConditionalHeaders(request.headers);
    const sanitizedHeaders = new Headers(sanitizedHeadersObj);

    const object = yield* getObject(bucket, resourcePath, {
      onlyIf: sanitizedHeaders,
      range: sanitizedHeaders,
    }).pipe(
      Effect.catchIf(
        (e) =>
          e.message.includes("if-none-match") ||
          e.message.includes("if-match") ||
          e.message.includes("ETag"),
        () => getObject(bucket, resourcePath, { range: sanitizedHeaders })
      )
    );
    ...
    // Note: object.size should always be defined per types, but can be undefined at runtime
    // We use explicit checks and fallbacks for safety
    const size = object.size || 0;
    if (object.range) {
      if ("suffix" in object.range) {
        const length = object.range.suffix || 0;
        const start = size - length;
        headers["Content-Range"] =
          `bytes ${String(start)}-${String(size - 1)}/${String(size)}`;
